...
import com.zige.robot.fsj.ui.album.widget.mode.LayDownMode;
import com.zige.robot.fsj.ui.album.widget.mode.PullOutMode;
import com.zige.robot.fsj.ui.album.widget.mode.StretchMode;
import com.zige.robot.fsj.ui.album.widget.mode.SwipeMode;
...
public class SwipeLayout extends FrameLayout {
    ...
    private OnSmoothScrollFinishedListener onSmoothScrollFinishedListener = new OnSmoothScrollFinishedListener() {
        ...
        public void onSmoothScrollFinished(Status status) {
            ...
            LogUtil.showW(TAG, "status:" + mStatus, TAG_ENABLE);
        }
    };
    ...
    protected void onFinishInflate() {
        ...
        mDragEdges.put(DragEdge.Left, getViewByIndex(leftViewIndex));
        mDragEdges.put(DragEdge.Right, getViewByIndex(rightViewIndex));
    }
    ...
    public View getLeftView() {
        return mDragEdges.get(DragEdge.Left);
    }

    public View getRightView() {
        return mDragEdges.get(DragEdge.Right);
    }
    ...
    public View pointToChild(int x, int y) {
        ...
        LogUtil.showW("phoneTest", "x:" + x, true);
        LogUtil.showW("phoneTest", "y:" + y, true);
        for (int i = count - 1; i >= 0; i--) {
            final View child = getChildAt(i);
            frame = getHitRect(child);
            LogUtil.showW("phoneTest", "left:" + frame.left + "  right:" + frame.right, true);
            LogUtil.showW("phoneTest", "top:" + frame.top + "  bottom:" + frame.bottom, true);
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        LogUtil.showW(TAG, "dispatchTouchEvent", TAG_ENABLE);
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        LogUtil.showW(TAG, "onInterceptTouchEvent", TAG_ENABLE);
        if (!mSwipeEnabled) {                                    //没开启侧滑功能时，直接返回，不拦截事件
            return super.onInterceptTouchEvent(event);
        }
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN: {
                LogUtil.showW(TAG, "interceptTouchEvent  ACTION_DOWN", TAG_ENABLE);
...
//                mLastMotionY = y;
            ...
            case MotionEvent.ACTION_MOVE: {
                LogUtil.showW(TAG, "interceptTouchEvent  ACTION_MOVE", TAG_ENABLE);
                final int x = (int) event.getX();
                final int y = (int) event.getY();
                final int xDiff = x - mFirstMotionX;
                final int yDiff = y - mFirstMotionY;
//                mLastMotionX = x;
//                mLastMotionY = y;

//                LogUtil.showW("fsj", "xDiff:" + xDiff);
//                LogUtil.showW("fsj", "yDiff:" + yDiff);
        ...
        LogUtil.showW("isIntercept:" + isIntercept);
        return isIntercept;
    }
    ...
    private void determineWhichDrageEdge(int xDiff) {
        LogUtil.showW(TAG, "determineWhichDrageEdge", TAG_ENABLE);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        LogUtil.showW(TAG, "onTouchEvent", TAG_ENABLE);
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN: {
                LogUtil.showW(TAG, "onTouchEvent  ACTION_DOWN", TAG_ENABLE);
                if (mMainView == null) {
                    return false;
                }
            ...
            case MotionEvent.ACTION_MOVE: {
                LogUtil.showW(TAG, "onTouchEvent  ACTION_MOVE", TAG_ENABLE);
                ...
                //只要拦截的情况下才偏移控件
                if (isIntercept) {
                    final int xDistance = x - mLastMotionX;
                    mLastMotionX = x;
                    if (isFirstAction) {
                        mFirstMotionX = (int) event.getX();
                        isFirstAction = false;
                        ...
                        LogUtil.showW(TAG, "mIsBeingDragged:" + mIsBeingDragged, TAG_ENABLE);
                        LogUtil.showW(TAG, "mCurrentDragEdge:" + mCurrentDragEdge, TAG_ENABLE);
                        LogUtil.showW(TAG, "state:" + mStatus, TAG_ENABLE);
                        if (mIsBeingDragged || mCurrentDragEdge != DragEdge.Empty) {
                            //当可拖拽时进行偏移量累加
                            checkOffset(xDistance);
                            final ViewParent parent = getParent();
                            if (parent != null) {
                                parent.requestDisallowInterceptTouchEvent(true);
                            }
            ...
            case MotionEvent.ACTION_UP:
            case MotionEvent.ACTION_CANCEL: {
                LogUtil.showW(TAG, "onTouchEvent  ACTION_UP", TAG_ENABLE);
                final ViewParent parent = getParent();
                if (parent != null) {
                    parent.requestDisallowInterceptTouchEvent(false);
                }
    ...
    private final void smoothScrollTo(Status status, int newScrollValue, int duration, int delayMillis,
        ...
        LogUtil.showW(TAG, "oldScrollValue:" + oldScrollValue, TAG_ENABLE);
    ...
    private void perforAdapterViewItemClick() {
        LogUtil.showW(TAG, "perforAdapterViewItemClick", TAG_ENABLE);
